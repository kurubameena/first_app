...
public class ChangeNewPassword extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		boolean flag=new UserDAO().changePassword(user,newpwd);
		if(flag)
		{
			
			response.sendRedirect("UserHome.jsp?auser="+user);
		}
		else
			response.sendRedirect("CreateNewPass.jsp?status=sorry operation failed&auser="+user);
		out.flush();
		out.close();
	}
